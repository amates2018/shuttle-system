...
import io.peanutsdk.animations.AnimationEndListener;
import io.peanutsdk.animations.FoldAnimation;
import io.peanutsdk.animations.HeightAnimation;

/**
 * Very first implementation of Folding Cell
 */
	...
	public FoldingCell(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		if (styledAttrs!=null){
			...
			for (int i = 0; i < count; ++i) {
				...
				if (attr == R.styleable.FoldingCell_cellAnimationDuration) {
					this.mAnimationDuration = styledAttrs.getInt(R.styleable.FoldingCell_cellAnimationDuration, DEF_ANIMATION_DURATION);
